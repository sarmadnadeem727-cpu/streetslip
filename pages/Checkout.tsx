...
const Checkout: React.FC = () => {
  ...
  const handleOrder = async (e: React.FormEvent) => {
    ...
    // STEP 1: Generate High-Res Order Ref Number
    const generatedId = `STSLIP-${Date.now()}`;
    ...
    try {
      ...
      console.error("Customer Notification Failed (Non-Fatal):", notificationError);
    }

    // STEP 4: Success Sequence / Receipt Transition
  ...
  const downloadReceipt = async () => {
    ...
    try {
      ...
      link.href = image;
      link.download = `STREETSLIPP_RECEIPT_${orderId}.png`;
      link.click();
    } catch (err) {
      ...
      <div className="min-h-screen pt-32 pb-20 px-4 flex flex-col items-center justify-center bg-black">
        <ErrorBoundary>
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full relative">
            ...
            <div ref={receiptRef} className="bg-white p-6 sm:p-10 shadow-[0_30px_80px_rgba(255,255,255,0.1)] relative overflow-hidden">
              <ThermalPrinterReceipt>
                ...
                {/* SHIP TO SECTION */}
                <div className="mb-10 text-[14px] space-y-1.5 p-5 border-2 border-black border-dashed bg-black/5">
                  <p className="font-black tracking-widest uppercase mb-3 text-base border-b border-black pb-2 text-black/90">SHIP TO</p>
                  <p className="font-bold uppercase tracking-wide">{formData.firstName} {formData.lastName}</p>
                  <p className="font-bold uppercase tech-mono tracking-wide">{formData.phone}</p>
                  <p className="font-bold uppercase tracking-wide break-all text-xs">{formData.email}</p>
                  <p className="font-bold uppercase text-black/80 tracking-wide">{formData.address}</p>
                  <p className="font-bold uppercase text-black/80 tracking-wide">{formData.city}, PK</p>
                </div>
                ...
                <div className="text-center space-y-4 text-[11px]">
                  ...
                  <div className="flex flex-col gap-3">
                    <span className="block w-full py-2 border-2 border-black text-black font-black uppercase hover:bg-black hover:text-white transition-colors text-center cursor-not-allowed">
                      TRACKING ASSIGNED SHORTLY
                    </span>
                  </div>
                  ...
                  <div className="pt-6">
                    ...
                    <p className="text-[9px] font-bold text-gray-500 mt-1 uppercase">Awaiting Logistics Assignment</p>
                  </div>
                </div>
              </ThermalPrinterReceipt>
            </div>
            ...
            <div className="mt-12 flex flex-col gap-4">
              <button
                ...
                className="w-full py-6 bg-[#00ff88] text-black text-center font-black text-2xl rounded-full hover:bg-[#00d4ff] hover:scale-105 transition-all uppercase tracking-widest italic bebas shadow-[0_0_30px_rgba(0,255,136,0.2)] flex items-center justify-center gap-3"
              >
                SAVE RECEIPT AS IMAGE
              </button>
              <Link to="/" className="w-full py-8 bg-white text-black text-center font-black text-3xl rounded-full hover:bg-gray-200 hover:scale-105 transition-all uppercase tracking-widest italic bebas shadow-[0_0_40px_rgba(255,255,255,0.3)]">Return to Vault</Link>
            </div>
          </motion.div>
        </ErrorBoundary>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12 sm:mb-20">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-3 px-6 py-2 bg-[#00d4ff]/10 text-[#00d4ff] rounded-full text-[10px] font-black uppercase tracking-[0.4em] w-fit italic">
            <Truck className="h-4 w-4" /> FAST NATIONWIDE DELIVERY
          </div>
        </div>
      </motion.div>
        ...
        <div className="lg:col-span-7">
          ...
          <AnimatePresence>
            {isSubmitting && (
              ...
              >
                <BoxTruckLoader />
                <p className="text-white font-mono mt-8 text-sm">Securely connecting to Logistics Grid...</p>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            <div className="glass p-8 sm:p-12 rounded-[2.5rem] border-[#00ff88]/20 bg-[#00ff88]/5">
              <div className="flex items-center gap-6 p-6 sm:p-10 bg-black/40 border-2 border-[#00ff88]/40 rounded-[2rem] sm:rounded-[3rem] shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs sm:text-base font-medium">Safe and secure payment at your doorstep. Fast Shipping.</p>
                </div>
              </div>
            </div>
