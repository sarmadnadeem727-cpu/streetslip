...
const Collection: React.FC = () => {
  ...
  const [filterType, setFilterType] = useState<'all' | 'low' | 'high' | 'top-selling'>('all');
  ...
  const products = useMemo(() => {
    ...
    if (filterType === 'top-selling') {
      return data.filter(p => p.id === 'vortex-shadow' || p.id === 'apex-drift');
    }
    return data;
  }, [filterType]);
            ...
            >
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  {['all', 'low', 'high', 'top-selling'].map((type) => (
                    <button
                      key={type}
                      onClick={() => setFilterType(type as any)}
                      className={`w-full text-left px-5 py-4 rounded-xl border transition-all uppercase tracking-wider text-sm font-bold ${filterType === type
                        ? 'bg-white text-black border-white'
                        : 'bg-white/5 border-white/10 text-gray-400 hover:border-white/30'
                        }`}
                    >
                      {type === 'all' ? 'Featured' : type === 'low' ? 'Price: Low to High' : type === 'high' ? 'Price: High to Low' : 'Top Selling'}
                    </button>
                  ))}
                </div>
              </div>
